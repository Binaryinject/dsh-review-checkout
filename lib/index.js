...
function apply(ctx) {
  ...
  async function doRevert(agentId, body, liveAgent) {    try {
      ...
      // root bucket — fall back to the same root resolution the review
      // endpoints use instead of reporting "no record".
      let files = sessions.get(agentId)
      if (!files) {
        const rootId = resolveRootId(agentId, liveAgent)
        if (rootId !== agentId) files = sessions.get(rootId)
      }
  ...
  // @deepseek-ai/dsh-tools is shipped by the HOST as its own dependency. A
  // `link:`-installed plugin has its real path OUTSIDE the profile, so the
  // ordinary node_modules walk never reaches it — and the published 0.1.5-rc.x
  // line cannot be installed from npm either (its transitive
  // @deepseek-ai/dsh-invariants range matches no released version: only
  // prereleases exist). So anchor a fallback on the RUNNING host executable:
  // realpath(process.argv[1]) is <dsh>/lib/bin.js, and <dsh>/node_modules is
  // exactly where the host keeps dsh-tools. Falls back to the plain specifier
  // first, and to the original error when no anchor resolves (Desktop / tests).
...
export {
  ...
  isSafeRecordedPath, latestWindowOf, loadSessions, loadStateInto, merge3, name,
