...
function apply(ctx) {
  ...
  // (observed: probe webServer=n on every dsh web start). ctx.inject below
  // registers a dependent fiber: the route attaches whenever the webServer
  // service becomes available, detaches if it goes away, and simply never
  // runs in compositions that provide no webServer (still optional).
