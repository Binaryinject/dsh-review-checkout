...
import { mkdirSync, readFileSync, renameSync, rmSync, writeFileSync, existsSync, appendFileSync, realpathSync } from 'node:fs'
...
/**
 * Locate a literal line fragment inside a file snapshot.
 *
...
function locateFragment(beforeText, fragmentText) {
  ...
  const fileLines = splitLines(beforeText)
  const needle = splitLines(fragmentText)
  ...
  for (let i = 0; i + needle.length <= fileLines.length; i++) {
    ...
    count++
    if (first < 0) first = i
    i += needle.length - 1 // non-overlapping scan, mirroring dsh-fs-local's countOccurrences
  }
  if (count === 0) return null
  return { line: first + 1, count }
}
...
function launchEditor(bin, args) {
  if (process.platform === 'win32' && /\.(cmd|bat)$/i.test(String(bin))) {
    const q = (s) => '"' + String(s).replace(/"/g, '""') + '"'
    const inner = [q(bin), ...args.map(q)].join(' ')
    execFileSync(process.env.ComSpec || 'cmd.exe', ['/d', '/s', '/c', '"' + inner + '"'], {
      timeout: 10000, stdio: 'ignore', windowsVerbatimArguments: true
    })
    return
  }
...
function apply(ctx) {
  ...
  ctx.on('tools/result', (exec, result) => {
    try {
      ...
      if (toolName === 'edit') {
        ...
        rec.ops.push({ kind: 'edit', at, turn, before, after, oldString: cap(input.old_string), newString: cap(input.new_string), replaceAll: input.replace_all === true })
      } else {
  ...
  // (locateFragment). When it can be recovered the gutter shows REAL file lines;
  // otherwise the numbering is the fragment-relative 1..N and the section is
  // flagged `lineExact: false` so the UI can mark it instead of passing it off as
  // absolute. `write` needs no recovery: its content IS the whole file, so 1..N
  // already are the real lines.
  function buildSections(ops, indices) {
    ...
    for (const i of indices) {
      ...
      if (op.kind === 'edit') {
        ...
        const located = locateFragment(op.before, typeof op.oldString === 'string' ? op.oldString : '')
        ...
        section = { kind: 'edit', at: op.at, hunks, truncated, lineExact: !!located, lineBase: located ? located.line : null }
