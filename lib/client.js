	...
	factory: (require) => {
		...
		function detailFromRec(rec) {
			...
			for (let i = 0; i < rec.ops.length; i++) {
				...
				sections.push({ kind: op.kind, at: op.at, hunks: hunks, wholeFile: op.kind === "write", opIndex: i, revertible: false, canUndo: false });
			}
		...
		function rebuildReview() {
			...
			reviewAsync = loadFullHistory(session).then((events) => {
				...
				else if (store.mode === "latest" && store.selected) loadDetail(store.selected);
			}).catch((e) => {
		...
		function loadLatest() {
			const session = store.currentSession;
			// When the latest window carries no turn/start, ops are tagged turn 0
			// (a fresh window) — treat them as the latest turn too.
			const turn = store.latestTurn || 0;
			...
			for (const rec of reviewFiles.values()) {
				...
				items.push({ path: rec.path, name: displayName(rec.path, rec.cwd), cwd: rec.cwd, ops: ops.length, writes: writes, edits: edits, added: added, removed: removed, lastTime: ops[ops.length - 1].at, revertible: false, sections: detailFromRec({ path: rec.path, ops: ops }).sections });
			}
			items.sort((x, y) => y.lastTime - x.lastTime);
			if (seq !== reqSeq || store.currentSession !== session) return;
			setState({ turnData: { turn: turn, files: items } });
		}
		...
		// Scope switch (此会话 / 最新一轮): resets the selection so the detail
		// pane never shows a file from the other scope.
		function setMode(mode) {
			setState({ mode: mode, selected: null, detail: null, loadingDetail: false });
			if (mode === "latest") loadLatest();
		}
		// Select a file for the two-pane review view. Latest mode renders the
		// turn payload's inline sections directly; session mode fetches the
		// file's full record (host channel → local history fallback).
		function selectFile(f) {
			if (!f || !f.path) return;
			if (store.mode === "latest") {
				setState({
					selected: f.path,
					detail: { path: f.path, sections: (f && f.sections) || [], revertible: false },
					loadingDetail: false,
					error: null
				});
			} else {
		...
		function TurnReview({ matched, sessionId, turn: turnLoc, seq, inputActions }) {
			...
			// 修复：collectTurn 对无匹配的轮号返回空对象（opCount=0），原 fallback
			// （|| windowSummary()）永不触发——只要 owner.turn.turn 的编号体系与 op.turn
			// （会话日志轮次）不一致，卡片就永远不渲染（cdxTurnDom=0）。
			// 现在依次降级：槽位轮号 → host 报告的最新日志轮次 → 窗口汇总兜底。
			let local = turnNo == null ? null : collectTurn(turnNo);
			if (!local || local.opCount === 0) local = collectTurn(latestTurn);
			if (!local || local.opCount === 0) local = windowSummary();
			try {
			} catch (e) {}
			// The tail slot renders a single instance per session window; the
			// window data IS the latest turn (turn numbers may mismatch), so
			// show whenever the window has changes.
			if (!local || local.opCount === 0) return null;
			const first = local.files[0];
			const title = local.files.length === 1 ? first.name : first.name + " 等 " + local.files.length + " 个文件";
			const go = () => {
				try { setState({ mode: "latest" }); } catch (e) {}
				openReviewTab();
			};
			return React.createElement("div", { className: "cdx-turn", style: { background: colors.turnBg, borderColor: colors.turnBorder }, onClick: go, role: "button", title: "打开审查视图" },
				...
				React.createElement("div", { className: "cdx-turn-main" },
					...
					React.createElement("div", { className: "cdx-turn-body" },
						React.createElement("div", { className: "cdx-turn-title", style: { color: themeLabel(dark) } }, "已编辑 " + title),
					...
					React.createElement("div", { className: "cdx-turn-actions" },
						React.createElement("button", {
							...
							onClick: (e) => {
								...
								const files = local.files.map((f) => f.path);
								const cmd = files.length === 1
									? '请撤回文件 "' + files[0] + '" 的全部修改（使用 diff_review_revert 工具）。'
									: '请撤回以下文件的全部修改（请依次使用 diff_review_revert 工具执行）：\n' + files.map((p) => '- ' + p).join('\n');
						...
						React.createElement("button", {
							...
							style: { color: themeLabel(dark) },
							onClick: () => {
								try { setState({ mode: "latest", reviewTurn: turnNo }); } catch (e) {}
								openReviewTab();
							}
						}, "审核"))),
				local.files.length > 1 ? React.createElement("div", { className: "cdx-turn-files" },
					local.files.map((f) => React.createElement("div", { key: f.path, className: "cdx-turn-file" },
						React.createElement("span", { className: "cdx-turn-file-name" }, f.name),
		...
		function CodexFileCard({ item, expanded, onToggle }) {
			const detail = expanded ? detailFromRec(reviewFiles.get(item.path)) : null;
			...
			const head = React.createElement("button", { type: "button", className: "cdx-file-head", onClick: onToggle },
				...
				React.createElement("span", { className: "cdx-file-path", title: item.path }, item.path),
			...
			return React.createElement("div", { className: "cdx-file", style: { background: colors.ctxBg } }, head, body);
		}
		...
		function ReviewView(props) {
			...
			const colors = useStore((s) => s.colors);
			const list = mode === "latest"
				? ((turnData && turnData.files && turnData.files.length) ? turnData.files : windowList(latestTurn))
				: (files || []);
			...
			// ── left pane: file list (click selects, right pane shows the diff) ──
			...
			const fileList = React.createElement("div", { className: "cdx-filelist" },
				...
				list.map(fileRow));
			// ── right pane: selected file's diff sections ──
			let detailBody;
			if (!selected) detailBody = React.createElement("div", { className: "cdx-empty" }, "在左侧选择文件查看修改对比");
			else if (loadingDetail) detailBody = React.createElement("div", { className: "cdx-empty" }, "加载中…");
				...
				const lang = langOf(detail.path || selected);
			...
			const detailPane = React.createElement("div", { className: "cdx-detail" },
				...
				React.createElement("div", { className: selected ? "cdx-detail-body" : null, style: selected ? { background: colors.ctxBg, border: "1px solid var(--dsw-alias-border-l2, rgba(128,128,128,0.3))", borderRadius: 8 } : null },
					detailBody));
			return React.createElement("div", { className: "drv-view" },
				...
				React.createElement("div", { className: "cdx-toolbar" },
					...
					React.createElement("div", { className: "cdx-mode", role: "group" },
						React.createElement("button", {
							className: "cdx-mode-btn" + (mode === "session" ? " cdx-active" : ""),
							onClick: () => setMode("session")
						}, "此会话"),
						React.createElement("button", {
							className: "cdx-mode-btn" + (mode === "latest" ? " cdx-active" : ""),
...
.cdx-turn-file { display:grid; grid-template-columns: minmax(0,1fr) auto; align-items:center; column-gap:8px; width:100%; padding:7px 14px; font-size:12px; }
...
/* two-pane review view: file list (left) + selected file's diff (right);
...
.cdx-file { border:1px solid var(--dsw-alias-border-l2, rgba(128,128,128,0.3)); border-radius:8px; overflow:hidden; background:var(--dsw-alias-surface-2, #161b22); flex-shrink:0; }
.cdx-file-head { display:flex; align-items:center; gap:8px; width:100%; padding:8px 10px; border:none; background:transparent; color:inherit; cursor:pointer; font-family:inherit; font-size:12.5px; text-align:left; }
...
.cdx-file-head:hover { background:rgba(128,128,128,0.08); }
