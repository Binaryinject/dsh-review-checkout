window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ReviewView(props) {
			...
			// ── turn switcher: every turn seen in the parsed history, newest
			// first. reviewTurn === null means "latest"; the chat turn-cards'
			...
			const turnLabelOf = (t) => t === 0 ? "本轮（未分轮）" : "第 " + t + " 轮";
			const pickTurn = (v) => {
				if ((reviewTurn == null ? "latest" : String(reviewTurn)) === v) return;
				setState({ reviewTurn: v === "latest" ? null : Number(v), reviewFile: null, selected: null, detail: null, loadingDetail: false });
			};
			// Chat turn-card jump: the turn switch happens via reviewTurn; here
			// we select the clicked file so the right pane shows its content.
			React.useEffect(() => {
				...
				setState({ selected: reviewFile, detail: null, loadingDetail: false });
			}, [reviewFile, turnData]);
			// ── left pane: file list of the reviewed turn ──
			const selItem = selected ? list.find((f) => f.path === selected) : null;
			const selectFile = (f) => {
				if (!f || !f.path) return;
				setState({
					selected: f.path,
					detail: { path: f.path, sections: (f.sections && f.sections.length) ? f.sections : detailFromRecTurn(reviewFiles.get(f.path), turnShown).sections }
				});
			};
			...
			const fileList = React.createElement("div", { className: "cdx-filelist" },
				...
				list.length === 0 ? React.createElement("div", { className: "cdx-empty" }, "该轮暂无文件修改") :
				list.map(fileRow));
			...
			else if (!selItem) detailBody = React.createElement("div", { className: "cdx-empty" }, "该轮没有此文件的修改记录");
			...
			return React.createElement("div", { className: "drv-view" },
				...
				React.createElement("div", { className: "cdx-toolbar" },
					...
					React.createElement("select", {
						...
						value: reviewTurn == null ? "latest" : String(reviewTurn),
						onChange: (e) => pickTurn(e.target.value),
						title: "选择要审查的轮次"
					},
