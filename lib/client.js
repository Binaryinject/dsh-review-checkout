window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ReviewView(props) {
			...
			// Chat turn-card jump: switch back to the turn scope, then select
			// the clicked file so the right pane shows its content. The old
			// remembered-focus guard broke the "全部修改" (all-scope) case:
			// after switching to the cumulative list, re-clicking a file that
			// was already focused once was treated as applied and no selection
			// happened — the file never expanded. The jump now waits for the
			// target turn's payload (so sections are the right turn's), then
			// applies unless the view already shows that file with that data;
			// the payload refresh (turnData) re-runs this effect, so a jump
			// issued before the turn arrived still lands.
			React.useEffect(() => {
				if (!reviewFile) return;
				const targetTurn = (reviewTurn == null || !Number.isFinite(reviewTurn)) ? (latestTurn || 0) : reviewTurn;
				if (jumpApplied(reviewScope, selected, reviewFile, turnShown, targetTurn)) return;
			...
			}, [reviewFile, reviewTurn, reviewScope, turnData]);
