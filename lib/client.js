		...
		function openFileAtLine(sessionId, path, cwd, line) {
			try {
				...
				if (!ed || !ed.id) {
					...
					setState({ error: "没有可用的打开方式：在顶栏「编辑器」里选一个外部编辑器（DSH 自带的内置预览也不可用）。" })
					return
				}
				const abs = resolveAbsPath(sessionId, path, cwd)
				apiOpenWithEditor(ed.id, abs, line != null ? line : null, null).then((v) => {
					...
					// undetected editor used to look exactly like a dead button), and still
					// land the user on the line through the shell's preview.
					const why = (v && v.error) ? String(v.error) : "宿主未返回成功"
					if (inApp()) setState({ error: "外部编辑器打开失败（" + why + "）；已在右侧内置预览中定位到该行。" })
					else setState({ error: "打开编辑器失败（" + ed.name + " · " + abs + "）：" + why })
				}).catch((e) => {
					const why = String((e && e.message) || e)
					if (inApp()) setState({ error: "外部编辑器请求失败（" + why + "）；已在右侧内置预览中定位到该行。" })
					else setState({ error: "打开编辑器请求失败：" + why })
				})
			} catch (e) {
...
.cdx-split { flex:1; display:flex; min-height:0; margin-top:10px; border:1px solid var(--dsw-alias-border-l2, rgba(128,128,128,0.3)); border-radius:8px; overflow:hidden; }
.cdx-filelist { width:270px; border-right:1px solid var(--dsw-alias-border-l2, rgba(128,128,128,0.3)); overflow:auto; overscroll-behavior:contain; flex-shrink:0; padding:6px 0; }
...
.cdx-fl-name { flex:1 1 auto; min-width:0; font-weight:500; word-break:break-all; }
...
.cdx-detail-path { flex:1 1 auto; min-width:0; font-size:12px; opacity:0.8; word-break:break-all; font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace; }
