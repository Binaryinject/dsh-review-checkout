window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// the whole list. Replace-not-merge: the list comes wholly from the
		// host or wholly from the local parse, so a file touched by both the
		// parent and a subagent is never counted twice. An empty/failed host
		// payload keeps the local result untouched (identical to the previous
		// behavior). Pure — smoke-tested directly.
		function adoptHostSummary(host, localFiles, localLatestTurn) {
			...
			return { files: hostFiles, latestTurn: Math.max(hostLatest, 0), fromHost: true };
		}
		...
		// TAB-BADGE face of the host window: NO in-flight-turn guard. The badge
		// counts "modifications you can review right now", so the host's latest
		// activity stays honest even when a fresh turn has not written anything
		// yet. Applying the pill's guard here made the badge vanish between
		// turns — and since subagent changes exist only in the host, the badge
		// then read 0 exactly whenever the user opened a new turn. Pure.
		...
		// A host-sourced list entry carries no op sections, and a subagent-only
		// file has no local record either — expanding it must go through the
		// host file endpoint (loadDetail) instead of rendering an empty body.
		// Pure — smoke-tested directly.
		function needHostDetail(scope, item, rec) {
		...
		function rebuildReview() {
			...
			reviewAsync = loadFullHistory(session).then((events) => {
				...
				setState({ files: s.files, latestTurn: s.latestTurn, activeTurn: parsed.activeTurn, loadingFiles: false, error: null });
				if (store.mode === "latest") loadTurn(store.reviewTurn);
				hostP.then((host) => {
					if (rid !== reviewRid || store.currentSession !== session) return;
					const adopted = adoptHostSummary(host, s.files, s.latestTurn);
					if (adopted.fromHost) setState({ files: adopted.files, latestTurn: adopted.latestTurn });
				});
		...
		function SessionProbe(props) {
			React.useEffect(() => {
				if (props.sessionId && store.currentSession !== props.sessionId) {
					...
					setState({ currentSession: props.sessionId, files: null, selected: null, detail: null, mode: "latest", turnData: null, hostWindow: null, latestTurn: 0, error: null, loadingFiles: true });
					updateRunning();
					refreshFromServer();
				}
			}, [props.sessionId]);
		...
		function TabLabel() {
			...
			// used only when the host has no data for this session.
			const badgeTurn = (activeTurn > 0) ? activeTurn : latestTurn;
			// Host latest-activity payload when it has files (subagent changes are
			// aggregated there and never in the local transcript); whole-source
			// replacement — never mixed with the local window count. NO in-flight
			// guard here: the badge is a pending-review counter, not a per-turn one.
			const hostItems = badgeHostItems(hostWindow);
			const count = hostItems
				? hostItems.length
				: (files && files.length ? windowList(badgeTurn).length : 0);
		...
		function CodexFileCard({ item, expanded, onToggle, turn, scope }) {
			...
			// entry the local parse never saw (a subagent-only file — the host
			// summary carries no op sections) loads its hunks from the host file
			// endpoint instead of rendering an empty body.
			const remote = useStore((s) => s.detail);
			const rec = reviewFiles.get(item.path);
			const needRemote = needHostDetail(scope, item, rec);
			const detail = expanded
				? ((item.sections && item.sections.length)
					? { path: item.path, sections: item.sections }
					: needRemote
						? (remote && remote.path === item.path ? remote : { path: item.path, sections: [] })
		...
		function ReviewView(props) {
			...
			React.useEffect(() => {
				if (props.sessionId) {
					if (store.currentSession !== props.sessionId) {
						reqSeq++;
						setState({ currentSession: props.sessionId, files: null, selected: null, detail: null, mode: "latest", turnData: null, hostWindow: null, latestTurn: 0, error: null, loadingFiles: true, reviewTurn: null, reviewFile: null });
					}
					loadSummary();
				}
			}, [props.sessionId]);
			...
			const selectFile = (f) => {
				if (!f || !f.path) return;
				// 'all' scope: a host-only entry (subagent file) has no local
				// record, so its sections must come from the file endpoint.
				const allScope = reviewScope === "all";
				const needRemote = needHostDetail(reviewScope, f, reviewFiles.get(f.path));
				setState({
					selected: f.path,
			...
			const cardList = React.createElement("div", { className: "cdx-list", ref: listRef },
				...
				list.map((f) => React.createElement(CodexFileCard, {
					...
					onToggle: () => {
						...
						// 'all' scope, no local record (host-only / subagent file):
						// fetch the hunks from the host file endpoint.
						if (willExpand && needHostDetail(reviewScope, f, reviewFiles.get(f.path))) loadDetail(f.path);
					}
				})));
			...
			else if (loadingDetail && needHostDetail(reviewScope, selItem, reviewFiles.get(selected))) detailBody = React.createElement("div", { className: "cdx-empty" }, "加载中…");
			...
			return React.createElement("div", { className: "drv-view" },
				...
				React.createElement("div", { className: "cdx-toolbar" },
					...
					React.createElement(Tip, { text: "清空记录" }, React.createElement("button", { className: "cdx-tool-btn", onClick: () => { apiClear(store.currentSession).then(() => { setState({ files: [], detail: null, selected: null, turnData: null, hostWindow: null, latestTurn: 0 }); }); } }, "✕"))),
