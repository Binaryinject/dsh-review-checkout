	...
	factory: (require) => {
		...
		function openFileAtLine(sessionId, path, cwd, line) {
			try {
				...
				// The shell's own opener (captured from the chat's turnTail props) shows
				// the file — and the line — in the right sidebar. It needs no host
				// config, so it is both the no-editor default and the fallback.
				const inApp = () => {
					try {
				...
				// Built-in first: the shell's own opener lands on the line in the sidebar
				// preview and needs no host config. The external-editor route below is a
				// last resort for compositions without that opener (its picker is gone).
				if (inApp()) { setState({ error: null }); return }
				if (!ed || !ed.id) {
					setState({ error: "没有可用的打开方式：当前组合没有侧栏预览，也无法启动外部编辑器。" })
					return
				}
				...
				apiOpenWithEditor(ed.id, abs, line != null ? line : null, null).then((v) => {
					...
					setState({ error: "打开编辑器失败（" + ed.name + " · " + abs + "）：" + why })
				}).catch((e) => {
					setState({ error: "打开编辑器请求失败：" + String((e && e.message) || e) })
				})
			} catch (e) {
		...
		function ReviewView(props) {
			...
			const fileRow = (f) => React.createElement("button", {
				...
				React.createElement(FileTypeBadge, { path: f.path }),
			...
			const fileList = React.createElement("div", { className: "cdx-filelist" },
			...
			const detailPane = React.createElement("div", { className: "cdx-detail" },
				selected ? React.createElement("div", { className: "cdx-detail-head" },
					React.createElement(FileTypeBadge, { path: selected }),
			...
			return React.createElement("div", { className: "drv-view" },
				...
				React.createElement("div", { className: "cdx-toolbar" },
					...
					React.createElement(Tip, { text: "清空记录" }, React.createElement("button", { className: "cdx-tool-btn", onClick: () => { apiClear(store.currentSession).then(() => { setState({ files: [], detail: null, selected: null, turnData: null, hostWindow: null, latestTurn: 0, filesFromHost: false }); }); } }, "✕"))),
				error ? React.createElement("div", { className: "cdx-error" }, error) : null,
				viewMode === "cards"
					? cardList
					: React.createElement("div", { className: "cdx-split" },
						fileList,
						detailPane));
		}
...
.cdx-filelist { flex:0 0 auto; width:min(270px, 46%); border-right:1px solid var(--dsw-alias-border-l2, rgba(128,128,128,0.3)); overflow:auto; overscroll-behavior:contain; padding:6px 0; }
@container (max-width: 520px) { .cdx-filelist { display:none; } }
...
.drv-editor-menu { position:absolute; top:calc(100% + 4px); right:0; z-index:20000; width:max-content; min-width:170px; max-width:320px; padding:4px; border:1px solid rgba(128,128,128,0.45); border-radius:8px; background:var(--dsw-alias-surface-2, #22272e); box-shadow:0 6px 18px rgba(0,0,0,0.35); max-height:260px; overflow:auto; }
...
`;
		function apply(ctx) {
			...
			// The external-editor picker is intentionally NOT registered: opening a file
			// goes through the shell's own opener (right-sidebar preview at the line),
			// which also owns the editor preference — one picker, not two.
