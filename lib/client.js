window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// Card-view expanded set: add `path` while keeping every other entry —
		// the shared memory between split/card modes (a file picked in split
		// view re-expands when switching to cards, and vice versa). Pure.
		function expandFile(expanded, path) {
			const n = Object.assign({}, expanded || {});
			if (path) n[path] = true;
			return n;
		}
		...
		function ReviewView(props) {
			...
			React.useEffect(() => {
				...
				const n = expandFile(expanded, reviewFile);
				if (viewMode === "cards") {
			...
			const selectFile = (f) => {
				...
				setState({
					...
					reviewFile: null,
					// 与卡片视图共享展开记忆：切到列表时该文件自动展开
					expanded: expandFile(expanded, f.path),
			...
			const cardList = React.createElement("div", { className: "cdx-list", ref: listRef },
				...
				list.map((f) => React.createElement(CodexFileCard, {
					...
					onToggle: () => {
						const n = Object.assign({}, expanded || {});
						if (n[f.path]) delete n[f.path]; else n[f.path] = true;
						setState({ expanded: n, selected: f.path });
					}
				})));
			...
			const switchView = (v) => {
				...
				setState({ viewMode: v, expanded: v === "cards" && selected ? expandFile(expanded, selected) : expanded });
			};
