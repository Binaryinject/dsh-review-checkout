window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ── official history API (ctx.connection.api.sessions.history) ───────
		// The stable browser API used by the community (e.g. dsh-fork-diff):
		// paged, read-only, returns raw SessionEvents incl. tool/call + tool/result.
		const HISTORY_PAGE = 100;
		const HISTORY_MAX_PAGES = 2; // latest window only — the review shows the newest turn
		...
		// v4: the 0.1.2+ harness serves gateway remotes (ctx.remote.session).
		// page() cuts message-aligned pages but carries the SAME raw SessionEvents
		// ({type, seq, time, data}) inside records[].event, so parseReviewEvents
		// works unchanged; the pre-0.1.2 api client verb stays as fallback below.
		function remoteSessionFace() {
			try {
				const svc = ctxRef && ctxRef.get ? ctxRef.get("remote.session") : null;
				return svc && typeof svc.page === "function" && typeof svc.list === "function" ? svc : null;
			} catch (e) { return null; }
		}
		// Tolerant unwrap for every envelope these faces ever served:
		// {result:{ok,value}} (old verb) | {ok,value} (remotes) | direct payload.
		...
		async function loadFullHistory(sessionId) {
			const face = remoteSessionFace();
			const api = face ? null : historyApi();
			if (!face && !api) throw new Error("history API unavailable");
			...
			let beforeSeq;
			if (face) {
				// page() requires a throughSeq cursor: the session list's projection
				// baseline carries the current last event seq (asOfSeq).
				const listValue = remoteValueOf(await face.list({}));
				...
				const throughSeq = Number.isFinite(asOf) ? asOf : -1;
				if (throughSeq < 0) return events;
				for (let pageNo = 0; pageNo < HISTORY_MAX_PAGES; pageNo++) {
					if (signal.aborted) break;
					const value = remoteValueOf(await face.page({
			...
			for (let page = 0; page < HISTORY_MAX_PAGES; page++) {
				...
				const value = resp && resp.result && resp.result.ok ? resp.result.value : null;
				if (!value || !Array.isArray(value.events)) {
					if (value && value.error) throw new Error(String(value.error.message || String(value.error)));
					throw new Error("history page failed: " + JSON.stringify(resp && resp.result));
				}
		...
		function TurnReview({ matched, sessionId, turn: turnLoc, seq }) {
					...
					React.createElement("div", { className: "cdx-turn-actions" },
						React.createElement("button", {
							...
							onClick: (e) => {
								...
								// 自动填入 composer 输入框（React 受控组件：原生 setter + input 事件）
								try {
									const ta = document.querySelector("textarea[class*=composer], textarea[class*=input], textarea");
									if (ta && !ta.disabled) {
										try {
											const desc = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(ta), "value");
											if (desc && desc.set) desc.set.call(ta, cmd); else ta.value = cmd;
										} catch (e3) { ta.value = cmd; }
		...
		function ReviewView(props) {
			...
			const list = (turnData && turnData.files && turnData.files.length) ? turnData.files : windowList(latestTurn);
			...
			const toggleAll = () => setExpanded((prev) => {
				const all = list.every((f) => prev[f.path]);
				const n = {};
			...
			return React.createElement("div", { className: "drv-view" },
				...
				React.createElement("div", { className: "cdx-toolbar" },
					...
					React.createElement("span", { className: "cdx-mode-btn" }, "最新一轮"),
				...
				React.createElement("div", { className: "cdx-list" },
					list.length === 0 && !error ? React.createElement("div", { className: "cdx-empty" }, "暂无修改记录（进程内通过写入 / 编辑工具产生的文件修改会出现在这里）") : null,
					list.map((f) => React.createElement(CodexFileCard, { key: f.path, item: f, expanded: !!expanded[f.path], onToggle: () => toggle(f.path) }))));
		}
