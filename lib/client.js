window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ── official history API (session/follow + session/page RPC) ─────────
		// DSH 更新后 `connection.api.sessions.history` 已移除：历史经官方 /api
		// RPC 读取——`session/follow` 流（opening snapshot 帧即最新事件页）与
		// `session/page` 一元分页（按 throughSeq 回读更早页）。
		...
		// Resolve the official session Remote namespace (`remote.session`), which
		// reuses the gateway's WebSocket / worker-local transport for both stream
		// and unary calls. Falls back to the raw connection RPC surface below.
		function sessionRemote() {
			try {
				if (ctxRef && ctxRef.get) {
					const r = ctxRef.get("remote.session");
					if (r && typeof r.follow === "function") return r;
				}
			} catch (e) {}
			return null;
		}
		async function loadFullHistory(sessionId) {
			...
			// Opening snapshot: the latest message-aligned event page (records are
			// already ordered by seq and include tool/call + tool/result).
			const stream = remote
				? remote.follow({ address: address, maxMessages: HISTORY_PAGE }, signal)
				: rpcOpen
					? rpcOpen("/api", "session/follow", { args: { address: address, maxMessages: HISTORY_PAGE } }, signal)
					: null;
			if (!stream) throw new Error("history API unavailable");
			...
			for await (const frame of stream) {
				...
				if (frame && frame.type === "snapshot") {
					...
					firstSeq = first && first.event ? first.event.seq : null;
					hasMore = !!frame.hasMore;
				}
				break; // only the opening snapshot; never consume the live tail
			}

			// Read one older page to preserve the old two-page window semantics.
			if (hasMore && firstSeq !== null && firstSeq > 0) {
				const request = { address: address, throughSeq: firstSeq - 1, maxMessages: HISTORY_PAGE };
				try {
					const page = remote
						? await remote.page(request, signal)
						: rpcCall
							? await rpcCall("/api", "session/page", { args: request }, signal)
							: null;
					const value = page && page.ok !== false ? page.value : null;
				...
				} catch (e) { /* older page is best-effort; the snapshot already covers the newest turn */ }
			}
			return events;
		}
