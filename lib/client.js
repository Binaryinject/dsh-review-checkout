	...
	factory: (require) => {
		...
		function ReviewView(props) {
			...
			const turnLabel = (reviewTurn != null && Number.isFinite(reviewTurn))
				...
				: "最新一轮";
			// ── left pane: navigation file list. Clicking a file expands its
			// card and scrolls it into view (the cards themselves stay the
			// canonical diff renderer — sticky headers and per-turn revert
			...
			const jumpToFile = (path) => {
				...
				if (listEl) {
					for (const c of listEl.querySelectorAll(".cdx-file")) {
						if (c.dataset.path === path) { el = c; break; }
					}
				}
				const n = {};
				n[path] = true;
			...
			const fileList = React.createElement("div", { className: "cdx-filelist" },
				...
				list.length === 0 ? React.createElement("div", { className: "cdx-empty" }, "暂无文件") :
				list.map(fileRow));
...
/* two-pane review view: navigation file list (left) + turn card list (right).
   The cards stay the canonical diff renderer; the left rail just jumps to them.
...
.cdx-split .cdx-list { margin-top:0; border:none; border-radius:0; }
