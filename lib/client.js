window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const store = {
			...
			mode: "latest", latestTurn: 0, activeTurn: 0, turnData: null,
		...
		function fetchTurnData(session, turnNo) {
			...
			const p = apiTurn(session, turn).then((v) => {
				...
				return (v && Array.isArray(v.files) && v.files.length) ? { turn: typeof v.turn === "number" ? v.turn : turn, files: v.files } : localTurnData(turn);
		...
		function TabLabel() {
			...
			// Same "current in-flight turn" rule as the pill: a brand-new turn with
			// no write/edit yet must not keep showing the previous turn's badge.
			const badgeTurn = (activeTurn > 0) ? activeTurn : latestTurn;
			const count = files && files.length ? windowList(badgeTurn).length : 0;
		...
		function ChangePill() {
			...
			const items = windowList(pillTurn);
		...
		function ReviewView(props) {
			...
			const selectFile = (f) => {
				...
				setState({
					...
					detail: reviewScope === "all"
						? { path: f.path, sections: detailFromRec(reviewFiles.get(f.path)).sections }
			...
			const cardList = React.createElement("div", { className: "cdx-list", ref: listRef },
				...
				list.map((f) => React.createElement(CodexFileCard, {
					...
					onToggle: () => {
						...
						setState({
							expanded: (expanded && expanded[f.path]) ? {} : focusFile(expanded, f.path),
							selected: f.path
						});
					}
				})));
